...
async def edit_txt(client, message: Message):
    ...
    try:
        await message.reply_document(
            ...
            caption="📥**𝗘𝗱𝗶𝘁𝗲𝗱 𝗕𝘆 ➤ sachin yadav nitin yadav**"
        )
    except Exception as e:
...
async def ytplaylist_to_txt(client: Client, message: Message):
    ...
    if videos:
        ...
        await message.reply_document(
            document=file_name, 
            caption=f"`{title}`\n\n📥 𝗘𝘅𝘁𝗿𝗮𝗰𝘁𝗲𝗱 𝗕𝘆 ➤ sachin yadav nitin yadav"
        )
        os.remove(file_name)
    else:
...
async def help_command(client: Client, msg: Message):
    help_text = (
        "`/start` - Start the bot⚡\n\n"
        "`/sky` - Download and upload files (sudo)🎬\n\n"
...
async def upload(bot: Client, m: Message):
    ...
    await editable.edit("**𝗘𝗻𝘁𝗲𝗿 𝗣𝘄 𝗧𝗼𝗸𝗲𝗻 𝗙𝗼𝗿 𝗣𝘄 𝗨𝗽𝗹𝗼𝗮𝗱𝗶𝗻𝗴 𝗼𝗿 𝗦𝗲𝗻𝗱 `no` 𝗙𝗼𝗿 𝗢𝘁𝗵𝗲𝗿𝘀**")
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            if "jw-prod" in url or "d1q5ugnejk3zoi.cloudfront.net" in url:
                cmd = f'yt-dlp -o "{name}.mp4" "{url}"'

            if "kgs-v2.akamaized.net" in url:
                cmd = f'yt-dlp -o "{name}.mp4" "{url}"'
        ...
        # Check if the response status is OK
            ...
            # Remove the PDF file after sending
                    ...
                    Show = f"**🔔𝗗𝗢𝗪𝗡𝗟𝗢𝗔𝗗𝗜𝗡𝗚🔔**\n\n**📝ɴᴀᴍᴇ » ** `{name}\n\n🔗ᴛᴏᴛᴀʟ ᴜʀʟ » {len(links)}\n\n❄ǫᴜᴀʟɪᴛʏ » {res}`\n\n**🔗ᴜʀʟ » ** `{url}`\n\n🤖𝗕𝗢𝗧 𝗠𝗔𝗗𝗘 𝗕𝗬 ➤ sachin yadav nitin yadav\n\n🙂 Nikal chutiya 🙂"
    ...
    await m.reply_text("**🥳Yadav ji hogaya  𝗗𝗼𝗻𝗲🥳**")
